...
def runBiswebCa2(ipDict,hpc=0,sing=1):
    ...
    acceptableTerms = ['pythonPath','calPreprocPath','signal','noise','opticalorder','createmcref','signalout','noiseout','workdir','mcrefsignal','mcrefnoise','mask','runoption','configfile']
    ...
    if sing == 1:
        ...
        cmd = ipDict['pythonPath'] + ' ' + ipDict['calPreprocPath'] + ' ' + ' '.join(['--%s %s' % kv for kv in ipDict.items() if not any(kv[0] == aT for aT in ['calPreprocPath','pythonPath'])])
        print(cmd)

    else:
        raise Exception('Variable sing must be 1 or 0')
...
if __name__ == '__main__':
    ...
    parser.add_argument('configFile',type=str,help='Path to preprocessing configuration file (yml)')
    parser.add_argument('singPath',type=str,help='Path to bisweb calcium preproc singularity file')
    parser.add_argument('--singSwitch',type=int,help='1 if singPath is singularity image, 0 if it is python path',default=1)
    parser.add_argument('--pythonPath',type=str,help='if running via python script instead of singularity image please give path to appropriate python installation',default='python3')
        ...
        for f in sorted(fs):
            if f == 'rawsignl.nii.gz' and all([ft in root for ft in ftags]):
                ...
                if all(checkIps2) and not os.path.isfile(lastFile):
                    ...
                    runBiswebCa2(ipDict2,hpc=hpc,sing=singSwitch)
                ...
                if len(spatialSignlFiles) == 3 and len(spatialNoiseFiles) == 3:
                    ...
                    if all(checkIps) and not os.path.isfile(lastFile):
                        ...
                        runBiswebCa2(ipDict,hpc=hpc,sing=singSwitch)
